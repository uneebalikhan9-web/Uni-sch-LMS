...
export default function TDTimetable({ timetable }) {
  ...
  const timetableSlot = { display:'flex', justifyContent:'space-between', alignItems:'center', padding:'14px 20px', background:'#fff', borderRadius:'16px', border:'1px solid #e2e8f0', marginBottom:'8px', boxShadow:'0 2px 8px rgba(0,0,0,0.03)', borderLeft:'4px solid var(--primary-color, #4f46e5)' };
  ...
  return (
    <div style={S.tableCard} className="table-container animate-fadeIn">
      <div style={S.tableHeader}>
        <div>
          <h2 style={S.tableTitle}><CalendarBlank size={28} weight="duotone" color="var(--primary-color, #4f46e5)" style={{ verticalAlign:'middle', marginRight:'12px' }} />Academic Schedule</h2>
        ...
        {hasEntries ? (
          Object.entries(grouped).map(([day, entries]) => entries.length > 0 && (
            <div key={day} style={daySection}>
              <h4 style={dayHeading}>
                <CalendarBlank size={18} weight="duotone" style={{ color: 'var(--primary-color, #4f46e5)' }} />
                {day}
              </h4>
              {entries.map(entry => (
